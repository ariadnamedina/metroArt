...
def busqueda_autor(nombre):
    autor = input("Ingrese el nombre del autor (ej. 'Vincent van Gogh'): ").strip()
    if not autor:
        print("Debe ingresar un nombre de autor.")
        return

    print(f"\nBuscando obras de '{autor}'...")
    busqueda = f"artistOrCulture=true&q={autor}"
    object_ids = nombre.objetos_id(busqueda)
    
    if not object_ids:
        print(f"No se encontraron obras para el autor '{autor}'.")
        return

    print(f"Se consiguieron {len(object_ids)} obras")
    buscar=int(input("Cuantas desea ver:\n"))
    
    for obj_id in object_ids[:buscar]:
        detalle = nombre.detalle_obra(obj_id)
        if detalle:
            print("-" * 20)
            print(detalle)
    print("-" * 20)
